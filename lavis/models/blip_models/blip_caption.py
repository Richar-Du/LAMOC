...
class BlipCaption(BlipBase):
    ...
    def forward_decoder(self, samples, image_embeds):
        ...
        # raw_text = samples["text_input"]        # 带有prompt的caption
    ...
    def generate(
        ...
        for i in range(batch_size):
            for j in range(num_captions):
                sample_idx = torch.randint(low = 1, high = encoder_out.size(1), size = (1,100))
                image_embeds.append(encoder_out[i][sample_idx])
        ...
        batch_captions = [captions[i:i+10] for i in range(0, len(captions), num_captions)]
        # captions = [output for output in outputs]
