...
class AOKVQATask(VQATask):
    def valid_step(self, model, samples):
        if samples['image'].device != model.device:
        ...
        for pred_answer, ques_id, gt_answer in zip(answers[0], question_id, gt_answers):
...
@registry.register_task("aok_vqa_choice")
class AOKVQAChoiceTask(VQATask):
    def valid_step(self, model, samples):
        answers = model.predict_answers(
            samples=samples,
            answer_list=self.answer_list,
        ...
        gt_answers = samples["direct_answers"]
        gt_choices = []
        for i, idx in enumerate(samples['correct_choice_idx']):
            gt_choices.append(samples['choices'][i][idx])
        # gt_choices = samples['choices'][samples['correct_choice_idx']]

        for pred_answer, ques_id, gt_answer, gt_choice in zip(answers, question_id, gt_answers, gt_choices):
            pred_qa_pairs.append(
                {"question_id": ques_id, "pred_ans": pred_answer, "gt_ans": gt_answer, 'gt_choices': gt_choice}
            )
    ...
    def _report_metrics(self, result_file, split):
        ...
        for res in results:
            ...
            pred = res["pred_ans"]
            gt_choices = res["gt_choices"]
            if pred == gt_choices:
                acc += 1

        accuracy = acc / len(res) * 100
        metrics = {"agg_metrics": accuracy, "acc": accuracy}
